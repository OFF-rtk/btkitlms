...
export default function CartPage() {
    ...
    function openSheet() {
        setFoundBook(null); setManualIsbn(""); setSheetError("");
        setIssueDays(7); setScanMode("scan"); setSheetOpen(true);
    }

    function closeSheet() { setSheetOpen(false); setFoundBook(null); setSheetError(""); }

    async function handleIsbnSearch() {
        if (!manualIsbn.trim()) return;
        setSearching(true); setFoundBook(null); setSheetError("");
        try {
            const res = await fetch("/api/books");
            const allBooks: Book[] = await res.json();
            const match = allBooks.find(b => b.isbn.toLowerCase() === manualIsbn.trim().toLowerCase());
            if (match) setFoundBook(match);
            else setSheetError("No book found with that ISBN.");
            ...
            <div className={`fixed z-[70] flex flex-col bg-[#0a0a0a] border-stone-900 shadow-2xl transition-transform duration-500 ease-in-out bottom-0 left-0 right-0 max-h-[92vh] rounded-t-[2.5rem] border-t md:top-0 md:bottom-auto md:right-0 md:left-auto md:h-screen md:w-[480px] md:max-h-none md:rounded-none md:border-l md:border-t-0 ${sheetOpen ? "translate-y-0 md:translate-x-0" : "translate-y-full md:translate-x-full"}`}>
                ...
                <div className="flex-1 overflow-y-auto p-8 scrollbar-hide">
                    ...
                    <div className="mb-8 flex border border-stone-900 p-1 bg-stone-950 font-sans">
                        <button onClick={() => setScanMode("scan")} className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-[10px] font-black uppercase tracking-[0.2em] transition-all ${scanMode === "scan" ? "bg-amber-900 text-amber-50 shadow-lg" : "text-stone-700"}`}>
                            <Camera size={14} /> Scan Barcode
                        </button>
                        <button onClick={() => setScanMode("manual")} className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-[10px] font-black uppercase tracking-[0.2em] transition-all ${scanMode === "manual" ? "bg-amber-900 text-amber-50 shadow-lg" : "text-stone-700"}`}>
