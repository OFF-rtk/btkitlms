                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-stone-900">
                                <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans">Cover</th>
                                <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans">Title</th>
                                <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans hidden md:table-cell">Author</th>
                                <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans text-center">Total</th>
                                <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans text-center">Avail.</th>
                                <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans hidden lg:table-cell">Collections</th>
                        ...
                        <tbody>
                            {books.filter((b) => {
                                ...
                                <tr key={book.id} className="border-b border-stone-900/40 hover:bg-stone-950/40 transition-colors group">
                                    <td className="py-4 pr-4">
                                        <div className="h-14 w-10 overflow-hidden border border-stone-800 shadow-md">
                                            <img src={book.cover_url || PLACEHOLDER} alt="" className="h-full w-full object-cover grayscale-[20%] group-hover:grayscale-0 transition-all duration-500" />
                                        </div>
                                    </td>
                                    <td className="py-4 pr-4">
                                        <p className="text-sm text-[#e8e4db] italic tracking-tight line-clamp-1">{book.title}</p>
                                        <p className="text-[9px] text-stone-600 font-mono tracking-widest mt-0.5">{book.isbn}</p>
                                    </td>
                                    <td className="py-4 pr-4 hidden md:table-cell">
                                        <p className="text-[10px] text-amber-800 font-sans font-black uppercase tracking-[0.12em]">{book.author}</p>
                                    </td>
                                    <td className="py-4 pr-4 text-center text-sm text-stone-400">{book.total_copies}</td>
                                    <td className="py-4 pr-4 text-center">
                                        <span className={`text-sm font-bold ${book.available_copies > 0 ? "text-emerald-600" : "text-red-600"}`}>
                                    ...
                                    <td className="py-4 pr-4 hidden lg:table-cell">
                                        <div className="flex flex-wrap gap-1.5">
                                            {(book.collections || []).map((c) => (
                                                <span key={c} className="px-2 py-0.5 text-[8px] font-black uppercase tracking-widest bg-amber-950/20 border border-amber-900/15 text-amber-700">
                                                    {c}
                                                </span>
                                            ))}
                                        </div>
                                    ...
                                    <td className="py-4">
                                        <button
                                            ...
                                            className="flex h-8 w-8 items-center justify-center text-stone-700 hover:text-amber-600 transition-colors"
                                        >
                                            <Edit2 size={14} />
                                        </button>
                                    </td>
