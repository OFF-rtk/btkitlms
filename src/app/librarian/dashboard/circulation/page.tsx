...
export default function CirculationPage() {
    ...
    return (
        <div className="w-full min-h-screen px-6 py-8 md:px-12 md:py-12 pb-32 font-serif selection:bg-amber-900/40">
            {/* Header */}
            <div className="mb-12 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between border-b border-stone-900/50 pb-8">
                <div>
            ...
            <div className="mb-16">
                <p className="px-1 mb-6 text-[9px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans flex items-center gap-2">
                    <ClipboardList size={12} className="text-amber-900" />
                    Pending Issue Requests ({pendingRequests.length})
                </p>

                {pendingRequests.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 bg-[#0d0d0d] border border-stone-900 shadow-inner text-center">
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-stone-900">
                                    <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans">Student</th>
                                    <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans">Book</th>
                                    <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans hidden md:table-cell">ISBN</th>
                                    <th className="pb-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans text-center">Days</th>
                                </tr>
                            </thead>
                            <tbody>
                                {pendingRequests.map((req) => (
                                    <tr key={req.id} className="border-b border-stone-900/40 hover:bg-stone-950/40 transition-colors">
                                        <td className="py-4 pr-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[9px] text-stone-600 font-mono tracking-widest">{req.students.roll_number}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 pr-4">
                                        ...
                                        <td className="py-4 text-center">
                                            <span className="inline-block px-3 py-1 text-[9px] font-black uppercase tracking-widest bg-amber-950/10 border border-amber-900/20 text-amber-600">
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-stone-900">
                                    <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans">Student</th>
                                    <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans">Book</th>
                                    <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans hidden md:table-cell">Issued</th>
                                    <th className="pb-4 pr-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans">Due Date</th>
                                    <th className="pb-4 text-[8px] uppercase tracking-[0.3em] text-stone-700 font-black font-sans text-center">Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                {activeLoans.map((loan) => {
                                    ...
                                    return (
                                        <tr key={loan.id} className="border-b border-stone-900/40 hover:bg-stone-950/40 transition-colors">
                                            <td className="py-4 pr-4">
                                                <div className="flex items-center gap-2">
                                                    <User size={12} className="text-stone-700 shrink-0" />
                                                    <div>
                                                        <p className="text-sm text-[#e8e4db] italic tracking-tight">{loan.students.full_name}</p>
                                                        <p className="text-[9px] text-stone-600 font-mono tracking-widest">{loan.students.roll_number}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-4 pr-4">
                                            ...
                                            <td className="py-4 pr-4">
                                                <p className={`text-sm font-serif ${isOverdue ? "text-red-500" : "text-stone-400"}`}>
                                                    {new Date(loan.due_date).toLocaleDateString()}
                                                </p>
                                            </td>
                                            <td className="py-4 text-center">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[8px] font-black uppercase tracking-widest border ${isOverdue ? "text-red-500 border-red-900/30 bg-red-950/10" : "text-emerald-500 border-emerald-900/30 bg-emerald-950/10"}`}>
