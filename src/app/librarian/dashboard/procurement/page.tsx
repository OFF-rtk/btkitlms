        ...
        <div className="w-full min-h-screen px-6 py-8 md:px-12 md:py-12 pb-32 font-serif selection:bg-amber-900/40">
            ...
            {requests.length === 0 && (
                <div className="mt-20 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="max-w-sm text-sm text-stone-500 font-sans leading-relaxed tracking-wide font-light">
                        All procurement requests have been processed. Check back later.
                    </p>
                </div>
            )}
            ...
            {requests.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {requests.map((req) => (
                        <div key={req.id} className="group bg-[#0d0d0d] border border-stone-900 p-5 shadow-inner hover:border-amber-900/30 transition-all">
                            <div className="flex gap-4 mb-5">
                                <div className="h-24 w-16 shrink-0 overflow-hidden border border-stone-800 shadow-xl">
                                    <img
                                ...
                                <div className="flex flex-col justify-center min-w-0">
                                    ...
                                    <p className="text-[9px] text-stone-600 font-mono tracking-widest mt-1">{req.isbn}</p>
                                </div>
                            </div>

                            {/* Reason snippet */}
                            {req.reason && (
                                <div className="mb-5 border-l-2 border-stone-800 pl-4 py-1">
                                    <p className="text-[11px] text-stone-500 font-serif italic line-clamp-2 leading-relaxed">&ldquo;{req.reason}&rdquo;</p>
                                </div>
                            )}

                            <div className="flex items-center justify-between">
                                <span className="text-[9px] text-stone-700 font-sans tracking-widest">
                                    {new Date(req.created_at).toLocaleDateString()}
                                </span>
                                <button
                                    onClick={() => openReview(req)}
                                    className="flex items-center gap-2 px-4 py-2 border border-amber-900/30 text-[9px] font-sans font-black uppercase tracking-[0.2em] text-amber-700 hover:bg-amber-900 hover:text-amber-50 transition-all"
                                >
                                    <Eye size={12} /> Review
                                </button>
                            </div>
